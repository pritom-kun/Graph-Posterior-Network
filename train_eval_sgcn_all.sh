...
run_script() {
    ...
    if [ "$1" == "ood" ]; then
        python3 "train_and_eval.py" with "configs/reference/ood_loc_gdk.yaml" "data.dataset=$2" "run.gpu=$3" &
    elif [ "$1" == "class" ]; then
        python3 "train_and_eval.py" with "configs/reference/classification_gdk.yaml" "data.dataset=$2" "run.gpu=$3" &
    else
